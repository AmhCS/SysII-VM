;;; Kernel with Kernel Size implemented and stack and heap set up to play nicely with paging.
...
__start:
	...
	SUBUS	%G2	%G5	%G0
	;; %G2 holds the size of the kernel in bytes
	DIV	%G2	%G2	0x1000
	;; %G2 holds the size of the kernel in bytes div 4KB page size
	ADDUS	*+Ker_size	%G2	6
	;; store in Ker_size the number of pages required for kernel code + 5 (0x5000 for heap & stack)

	MUL	%G2	*+Ker_size	0x1000
	;; %G2 holds the number of bytes in the kernel's pagespace
	
	ADDUS	%SP	%G2	*+Ker_b ;initialize the stack at the limit of the kernel's pagespace
	COPY 	%FP 	%SP
	COPY	*+stack_end	%SP
...
f_3ROM:	ADDUS 	%G1 	%G1 	12
	...
	COPY	%G4	*+stack_end 	;lets start the program here
...
l_end:	COPY 	%G1	%G4		;%G1 holds the ROM3_end in RAM
	...
	SUBUS	*+heap_start	*+stack_end	0x5000
	;; store the base of the first page of the heap in heap_start
...
Kernel_Size:
;;; assumes that when coming in:
	;; %SP & %FP point to PFP
	;; %SP + 4 points to return address
	;; %SP + 8 points to return value space

	;; preserve %G0
	SUBUS	%SP	%SP	4
	COPY	*%SP	%G0
	
	;; set return value to kernel size
	ADDUS	%G0	%FP	8
	COPY	*%G0	*+Ker_size
